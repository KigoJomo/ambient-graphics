...
export default function CatalogPage() {
  return (
    <div className="catalog-page grid gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
      {collections.map((collection) => (
        <div key={collection.slug} className="collection p-4 bg-white rounded shadow">
          ...
          <Link href={`/catalog/${collection.slug}`}>
            <a className="inline-block text-center px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
              View More
            </a>
          </Link>
        </div>
